...
public class RouteGuideServer {
	...
	private static class RouteGuideService extends RouteGuideGrpc.RouteGuideImplBase {
		...
		/**
		 * Gets the {@link Feature} at the requested {@link Point}. If no
		 * feature at that location exists, an unnamed feature is returned at
		 * the provided location.
		 *
		 * @param request
		 *            the requested location for the feature.
		 * @param responseObserver
		 *            the observer that will receive the feature at the
		 *            requested point.
		 */
		@Override
		...
		/**
		 * Gets all features contained within the given bounding
		 * {@link Rectangle}.
		 *
		 * @param request
		...
		/**
		 * Gets a stream of points, and responds with statistics about the
		...
		public StreamObserver<Point> recordRoute(final StreamObserver<RouteSummary> responseObserver) {
			return new StreamObserver<Point>() {
				...
				public void onNext(Point point) {
					...
					// For each point after the first, add the incremental
					// distance from the previous point to
				...
				public void onCompleted() {
					...
					responseObserver.onNext(RouteSummary.newBuilder().setPointCount(pointCount)
							.setFeatureCount(featureCount).setDistance(distance).setElapsedTime((int) seconds).build());
					responseObserver.onCompleted();
				}
			};
		}
		...
		/**
		 * Receives a stream of message/location pairs, and responds with a
		 * stream of all previous messages at each of those locations.
		 *
		 * @param responseObserver
		 ...
		 * formula. The formula is based on
