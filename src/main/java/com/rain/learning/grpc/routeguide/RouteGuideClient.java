...
import com.rain.learning.grpc.routeguide.RouteGuideGrpc.RouteGuideBlockingStub;
import com.rain.learning.grpc.routeguide.RouteGuideGrpc.RouteGuideStub;
...
public class RouteGuideClient {
	...
	public void getFeature(int lat, int lon) {
		...
		if (RouteGuideUtil.exists(feature)) {
			info("Found feature called \"{0}\" at {1}, {2}", feature.getName(),
	...
	/**
	 * Blocking server-streaming example. Calls listFeatures with a rectangle of
	 * interest. Prints each response feature as it arrives.
	 */
	...
	/**
	 * Async client-streaming example. Sends {@code numPoints} randomly chosen
	 * points from {@code
	 * features} with a variable delay in between. Prints the statistics when
	...
	/**
	 * Bi-directional example, which can only be asynchronous. Send some chat
	 * messages, and print any chat messages that are sent from the server.
	 */
	public CountDownLatch routeChat() {
		...
		try {
			...
			for (RouteNote request : requests) {
				info("Sending message \"{0}\" at {1}, {2}", request.getMessage(), request.getLocation().getLatitude(),
	...
	/**
	 * Only used for unit test, as we do not want to introduce randomness in
	 * unit test.
	 */
	@VisibleForTesting
